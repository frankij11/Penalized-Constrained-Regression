...
The `penalized-constrained` package provides comprehensive diagnostics for assessing model fit quality.

```{python}
#| label: model-diagnostics

# Generate diagnostic report for PCReg-GCV model
report = pcreg.generate_summary_report(
    pc_gcv, X_train, y_train,
    full=True,
    gdf_method='gaines',  # Use Gaines' GDF for constrained models
    bootstrap=False,  # Skip bootstrap for faster rendering
    random_state=42
)
...
For the PCReg-GCV model: GDF = `{python} f'{gdf:.1f}'` with `{python} n_active` active constraints.

**Note on nonlinear optimization**: PCReg uses numerical optimization (scipy's SLSQP or trust-constr), which means classical regression assumptions (closed-form solutions, exact t-statistics) don't directly apply. The package provides bootstrap-based inference as a robust alternative.

```{python}
#| label: save-diagnostic-report

# Save HTML diagnostic report for interactive exploration
report.to_html('output_v2/pcreg_diagnostic_report.html', X=X_train, y=y_train)
```

An interactive HTML diagnostic report has been generated at `output_v2/pcreg_diagnostic_report.html`.
...
2. **If sample size ≤ 10 lots**: **Prefer PCReg-GCV** (more stable)
...
for model in summary_all.index:
    ...
    print(f"| {model} | {row['test_mape']:.4f} | {row['test_sspe']:.4f} | {row['b_error']:.4f} | {row['c_error']:.4f} | {row['r2']:.3f} |")
```

: Overall model performance across all 8,100 scenarios {#tbl-all-results}
